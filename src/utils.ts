 ...
 */
export function resolveTargets(target: any): any[] {
  if (!target) return [];
 ...
 */
export function drawSVG(target: any, vars: TweenVars): Tween {
  const targets = resolveTargets(target);
  const path = targets.length > 0 ? targets[0] : null;

  if (!path || typeof (path as any).getTotalLength !== "function") {
    throw new Error(
